...
// @include        http://klavogonki.ru/forum/moderators*
// @author         Fenex
// @version        2.0.8 KTS
...
function invert_topic() {
	...
	for (i=0;i<mchks.length;i++)
		if(mchks[i].getElementsByTagName('input')[0])
...
function uncheck(id) {
	document.getElementsByName('post['+id+']')[0].checked = false;
	count_posts();
}
function count_posts() {
	...
	for (k=0;k<e.length;k++) {
		if(e[k].getElementsByTagName('input')[0]&&e[k].getElementsByTagName('input')[0].checked) {
...
if(!document.getElementById('KTS_ModerTools')) {
	//title-moder
	...
	tmp_elem.setAttribute('id', 'KTS_ModerTools');
