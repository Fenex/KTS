	...
	chrome.extension.sendRequest({reason: "getID", username: username},
		function(response) {
			...
			if(!response.id)
				...
				t = 'ID: <span style="cursor:text;">'+ response.id.toString()+'</span> | <a class="getID_links" title="Профиль"><img path="/profile/'+response.id.toString()+'" src="'+chrome.extension.getURL('/img/icon/profile.png')+'" /></a> <a class="getID_links" title="Статистика"><img path="/profile/'+response.id.toString()+'/stats/" src="'+chrome.extension.getURL('/img/icon/sprint.png')+'" /></a> <a class="getID_links" title="Комментарии"><img path="/profile/'+response.id.toString()+'/comments/" src="'+chrome.extension.getURL('/img/icon/usefuladdons.png')+'" /></a>';
