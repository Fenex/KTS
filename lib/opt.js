...
var animHelp = {
	...
	setColor: function() {
		...
		document.querySelectorAll('#head .menu a')[document.querySelectorAll('#head .menu a').length - 1].style.color = c;
	},
...
function slowHide(d) {
	d++;
    if(d==256) {
        document.getElementById('checkbox_status_saved').innerHTML = '&nbsp;';
        return;
    }
    var str = d.toString(16);
    if(str.length==1)
        str = '0' + str;
    str += str + str;
    document.getElementById('checkbox_status_saved').style.color = '#' + str;
    q1 = setTimeout( function() { slowHide(d) }, 12);
}

function showAnswer(answer) {
	if(answer=='ok') {
		document.getElementById('checkbox_status_saved').innerHTML = 'Настройки сохранены.';
        document.getElementById('checkbox_status_saved').style.color = '#000000';
	} else {
		document.getElementById('checkbox_status_saved').innerHTML = 'Ошибка сохранения.';
        document.getElementById('checkbox_status_saved').style.color = 'red';
	}
	if(q1)
		clearTimeout(q1);
    if(answer=='ok')    
...
function getCheckedStyle() {
	var style = 'beige';
	if (document.getElementById('KTS_style_radio_blue').checked) {
		style = 'blue';
	}
	else if (document.getElementById('KTS_style_radio_gray').checked) {
		style = 'gray';
	}
	else if (document.getElementById('KTS_style_radio_pink').checked) {
		style = 'pink';
	}
	else if (document.getElementById('KTS_style_radio_green').checked) {
		style = 'green';
	}
